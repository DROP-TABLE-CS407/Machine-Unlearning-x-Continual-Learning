...
class Net(nn.Module):
    ...
    def unlearn(self, algorithm, t, x1, x2, alpha = 0.9):
        ...
        if algorithm == 'neggem':
            ...
            self.zero_grad()
            loss = self.ce(
                        self.forward(self.unlearn_memory_data[past_task][x1:x2], past_task)[:, offset1: offset2], self.unlearn_memory_labs[past_task][x1:x2] - offset1)
            ...
            for tt in range(t + 1):
                ...
                ptloss = self.ce(
                        self.forward(
                            self.unlearn_memory_datamemory_data[past_task],
                            past_task)[:, offset1: offset2],
                        self.unlearn_memory_labs[past_task] - offset1)
